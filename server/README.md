...
* Range Question (Convert From) - Metric Units:
    ...
    * `[#-#Unit]` (no steps defined, defaults to 1. Intended setting for survey questions)
...
* Range Question (Survey) - Imperial Units:
    * `[#-#UNIT(1)r]` (simple, nearest whole number)
    * `[#-#UNIT(0.5)r]` (convert to an answer rounded to 0.5 units)
...
* Q: `Starting at what height would you describe an adult man to be 'very tall'? [72-79in(1)r]` A: `[m]` User's Survey Answer: `75in`
